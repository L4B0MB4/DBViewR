...
const initialState: TableDataState = {
    tableWithData: [{
        ...
        Values: [[1, "hallo", new Date()], [2, "hallo2", new Date()]]
    }]
}
...
export const ermSlice = createSlice({
    ...
    reducers: {
        addTableData: (state, action: PayloadAction<TableWithData>) => {
            const index = state.tableWithData.findIndex(x => x.Name == action.payload.Name && x.Schema == action.payload.Schema)
            if (index < 0)
                state.tableWithData.push(action.payload)
            else
                state.tableWithData[index].Values = action.payload.Values
        },
    },
})
